...
# no ice
   ...
   f = Dataset(dirname + filename, 'r')
   OCNFRAC = f.variables['OCNFRAC'][:,:,:].data.squeeze()
   f.close()

   mask_var = OCNFRAC.copy()
   mask_var[mask_var<1.] = np.nan

   factor0 = 1.

   mask_control = np.zeros((12,ny,nx))
   mask_co2xp25 = np.zeros((12,ny,nx))
   mask_co2xp5 = np.zeros((12,ny,nx))
   mask_co2x1 = np.zeros((12,ny,nx))
   mask_co2x2 = np.zeros((12,ny,nx))
   mask_co2x3 = np.zeros((12,ny,nx))
   mask_co2x4 = np.zeros((12,ny,nx))
   mask_co2x5 = np.zeros((12,ny,nx))
   mask_co2x6 = np.zeros((12,ny,nx))
   mask_co2x7 = np.zeros((12,ny,nx))
   mask_co2x8 = np.zeros((12,ny,nx))
   
   for NM in range(12):
       dirname = '/data1/yliang/co2_experiments/processed_data/'
       filename = 'ICEFRAC_month' + str(NM+1) + '_mean_temp_output.nc'
       print(filename)
       f = Dataset(dirname + filename, 'r')
       mask_control[NM,:,:] = np.nanmean(f.variables['var_co2x1'][-30:,:,:].data*factor0, axis=0)
       mask_co2xp25[NM,:,:] = np.nanmean(f.variables['var_co2xp25'][-30:,:,:].data*factor0, axis=0)
       mask_co2xp5[NM,:,:] = np.nanmean(f.variables['var_co2xp5'][-30:,:,:].data*factor0, axis=0)
       mask_co2x1[NM,:,:] = np.nanmean(f.variables['var_co2x1'][-30:,:,:].data*factor0, axis=0)
       mask_co2x2[NM,:,:] = np.nanmean(f.variables['var_co2x2'][-30:,:,:].data*factor0, axis=0)
       mask_co2x3[NM,:,:] = np.nanmean(f.variables['var_co2x3'][-30:,:,:].data*factor0, axis=0)
       mask_co2x4[NM,:,:] = np.nanmean(f.variables['var_co2x4'][-30:,:,:].data*factor0, axis=0)
       mask_co2x5[NM,:,:] = np.nanmean(f.variables['var_co2x5'][-30:,:,:].data*factor0, axis=0)
       mask_co2x6[NM,:,:] = np.nanmean(f.variables['var_co2x6'][-30:,:,:].data*factor0, axis=0)
       mask_co2x7[NM,:,:] = np.nanmean(f.variables['var_co2x7'][-30:,:,:].data*factor0, axis=0)
       mask_co2x8[NM,:,:] = np.nanmean(f.variables['var_co2x8'][-30:,:,:].data*factor0, axis=0)
       f.close()

   mask_control[mask_control<0.15*factor0] = 0.
   mask_co2x1[mask_co2x1<0.15*factor0] = 0.
   mask_co2x2[mask_co2x2<0.15*factor0] = 0.
   mask_co2x3[mask_co2x3<0.15*factor0] = 0.
   mask_co2x4[mask_co2x4<0.15*factor0] = 0.
   mask_co2x5[mask_co2x5<0.15*factor0] = 0.
   mask_co2x6[mask_co2x6<0.15*factor0] = 0.
   mask_co2x7[mask_co2x7<0.15*factor0] = 0.
   mask_co2x8[mask_co2x8<0.15*factor0] = 0.

   mask_control[mask_control>=0.15*factor0] = np.nan
   mask_co2x1[mask_co2x1>=0.15*factor0] = np.nan
   mask_co2x2[mask_co2x2>=0.15*factor0] = np.nan
   mask_co2x3[mask_co2x3>=0.15*factor0] = np.nan
   mask_co2x4[mask_co2x4>=0.15*factor0] = np.nan
   mask_co2x5[mask_co2x5>=0.15*factor0] = np.nan
   mask_co2x6[mask_co2x6>=0.15*factor0] = np.nan
   mask_co2x7[mask_co2x7>=0.15*factor0] = np.nan
   mask_co2x8[mask_co2x8>=0.15*factor0] = np.nan

   mask_control[mask_control==0.] = 1.
   mask_co2x1[mask_co2x1==0.] = 1.
   mask_co2x2[mask_co2x2==0.] = 1.
   mask_co2x3[mask_co2x3==0.] = 1.
   mask_co2x4[mask_co2x4==0.] = 1.
   mask_co2x5[mask_co2x5==0.] = 1.
   mask_co2x6[mask_co2x6==0.] = 1.
   mask_co2x7[mask_co2x7==0.] = 1.
   mask_co2x8[mask_co2x8==0.] = 1.

#   mask_control_tmp = np.mean(mask_control, axis=0).copy()
#   mask_co2x1_tmp = np.mean(mask_co2x1, axis=0).copy()
#   mask_co2x2_tmp = np.mean(mask_co2x2, axis=0).copy()
#   mask_co2x3_tmp = np.mean(mask_co2x3, axis=0).copy()
#   mask_co2x4_tmp = np.mean(mask_co2x4, axis=0).copy()
#   mask_co2x5_tmp = np.mean(mask_co2x5, axis=0).copy()
#   mask_co2x6_tmp = np.mean(mask_co2x6, axis=0).copy()
#   mask_co2x7_tmp = np.mean(mask_co2x7, axis=0).copy()
#   mask_co2x8_tmp = np.mean(mask_co2x8, axis=0).copy()

   d1 = mask_control.shape[0]
   d2 = mask_control.shape[1]
   d3 = mask_control.shape[2]  

   for NT in range(d1):
       mask_control[NT,:,:] = (mask_control[NT,:,:]*mask_var).copy()
       mask_co2x1[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy() 
       mask_co2x2[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy() 
       mask_co2x3[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy()  
       mask_co2x4[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy() 
       mask_co2x5[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy()  
       mask_co2x6[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy()  
       mask_co2x7[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy() 
       mask_co2x8[NT,:,:] = (mask_co2x1[NT,:,:]*mask_var).copy() 
...
# simulated sat
   ...
   for NT in range(year_N):
       for NM in range(12):
           ...
           ts_sat_control_arctic[NT,NM] = np.nansum(sat_control[NT,NM,y1:,:]*area[y1:,:]*mask_control[NM,y1:,:])/np.nansum(area[y1:,:]*mask_control[NM,y1:,:])
           ...
           ts_sat_co2x1_arctic[NT,NM] = np.nansum(sat_co2x1[NT,NM,y1:,:]*area[y1:,:]*mask_co2x1[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x1[NM,y1:,:])
           ts_sat_co2x2_arctic[NT,NM] = np.nansum(sat_co2x2[NT,NM,y1:,:]*area[y1:,:]*mask_co2x2[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x2[NM,y1:,:])
           ts_sat_co2x3_arctic[NT,NM] = np.nansum(sat_co2x3[NT,NM,y1:,:]*area[y1:,:]*mask_co2x3[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x3[NM,y1:,:])
           ts_sat_co2x4_arctic[NT,NM] = np.nansum(sat_co2x4[NT,NM,y1:,:]*area[y1:,:]*mask_co2x4[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x4[NM,y1:,:])
           ts_sat_co2x5_arctic[NT,NM] = np.nansum(sat_co2x5[NT,NM,y1:,:]*area[y1:,:]*mask_co2x5[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x5[NM,y1:,:])
           ts_sat_co2x6_arctic[NT,NM] = np.nansum(sat_co2x6[NT,NM,y1:,:]*area[y1:,:]*mask_co2x6[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x6[NM,y1:,:])
           ts_sat_co2x7_arctic[NT,NM] = np.nansum(sat_co2x7[NT,NM,y1:,:]*area[y1:,:]*mask_co2x7[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x7[NM,y1:,:])
           ts_sat_co2x8_arctic[NT,NM] = np.nansum(sat_co2x8[NT,NM,y1:,:]*area[y1:,:]*mask_co2x8[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x8[NM,y1:,:])
...
#   plt.ylim(-1,13)
   plt.title('(a) Ice-free Arctic SAT Response')
   plt.ylabel('K')
...
# land-only
   ...
   for NT in range(year_N):
       for NM in range(12):
           print(NT, NM)
           ts_flx_control_global[NT,NM] = np.nansum(flx_control[NT,NM,y1:,:]*area[y1:,:]*mask_control[NM,y1:,:])/np.nansum(area[y1:,:]*mask_control[NM,y1:,:])
           ...
           ts_flx_co2x1_global[NT,NM] = np.nansum(flx_co2x1[NT,NM,y1:,:]*area[y1:,:]*mask_co2x1[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x1[NM,y1:,:])
           ts_flx_co2x2_global[NT,NM] = np.nansum(flx_co2x2[NT,NM,y1:,:]*area[y1:,:]*mask_co2x2[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x2[NM,y1:,:])
           ts_flx_co2x3_global[NT,NM] = np.nansum(flx_co2x3[NT,NM,y1:,:]*area[y1:,:]*mask_co2x3[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x3[NM,y1:,:])
           ts_flx_co2x4_global[NT,NM] = np.nansum(flx_co2x4[NT,NM,y1:,:]*area[y1:,:]*mask_co2x4[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x4[NM,y1:,:])
           ts_flx_co2x5_global[NT,NM] = np.nansum(flx_co2x5[NT,NM,y1:,:]*area[y1:,:]*mask_co2x5[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x5[NM,y1:,:])
           ts_flx_co2x6_global[NT,NM] = np.nansum(flx_co2x6[NT,NM,y1:,:]*area[y1:,:]*mask_co2x6[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x6[NM,y1:,:])
           ts_flx_co2x7_global[NT,NM] = np.nansum(flx_co2x7[NT,NM,y1:,:]*area[y1:,:]*mask_co2x7[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x7[NM,y1:,:])
           ts_flx_co2x8_global[NT,NM] = np.nansum(flx_co2x8[NT,NM,y1:,:]*area[y1:,:]*mask_co2x8[NM,y1:,:])/np.nansum(area[y1:,:]*mask_co2x8[NM,y1:,:])

           ts_sat_control_arctic[NT,NM] = np.nansum(sat_control[NT,NM,y1:,:]*area[y1:,:]*mask_control[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2xp25_arctic[NT,NM] = np.nansum(sat_co2xp25[NT,NM,y1:,:]*area[y1:,:]*mask_var[y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2xp5_arctic[NT,NM] = np.nansum(sat_co2xp5[NT,NM,y1:,:]*area[y1:,:]*mask_var[y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x1_arctic[NT,NM] = np.nansum(sat_co2x1[NT,NM,y1:,:]*area[y1:,:]*mask_co2x1[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x2_arctic[NT,NM] = np.nansum(sat_co2x2[NT,NM,y1:,:]*area[y1:,:]*mask_co2x2[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x3_arctic[NT,NM] = np.nansum(sat_co2x3[NT,NM,y1:,:]*area[y1:,:]*mask_co2x3[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x4_arctic[NT,NM] = np.nansum(sat_co2x4[NT,NM,y1:,:]*area[y1:,:]*mask_co2x4[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x5_arctic[NT,NM] = np.nansum(sat_co2x5[NT,NM,y1:,:]*area[y1:,:]*mask_co2x5[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x6_arctic[NT,NM] = np.nansum(sat_co2x6[NT,NM,y1:,:]*area[y1:,:]*mask_co2x6[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x7_arctic[NT,NM] = np.nansum(sat_co2x7[NT,NM,y1:,:]*area[y1:,:]*mask_co2x7[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
           ts_sat_co2x8_arctic[NT,NM] = np.nansum(sat_co2x8[NT,NM,y1:,:]*area[y1:,:]*mask_co2x8[NM,y1:,:])#/np.nansum(area[y1:,:]*mask_var[y1:,:])
...
#   plt.ylim(-5,35)
   plt.ylabel('W/m$^2$')
   plt.title('(c) Ice-free Turbulent Heat Flux Response')
