...
# read grid basics
   ...
   filename = varname + '_annual_zonal_mean_temp_output_e_case.nc'
...
#   plt.plot(np.nanmean(a_co2x7, axis=0)*factor,lev,'-',color='tomato',label='x7')
#   plt.plot(np.nanmean(a_co2x8, axis=0)*factor,lev,'-',color='red',label='x8')
   plt.yticks((np.array([1000,950,900,850,800,700,600,500,400,300,200,100,10])),['1000','950','900','850','800','700','600','500','400','300','200','100','10'], fontsize=9)
   plt.xticks([-5,0,5,10,15,20], fontsize=9)
   plt.legend(fontsize='small',ncol=1)
   plt.axis([-5, 22, 10, 1000])
   plt.gca().invert_yaxis()
   plt.title('(f) Arctic T Response')  
