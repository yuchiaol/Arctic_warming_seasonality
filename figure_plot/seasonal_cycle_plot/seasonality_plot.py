...
#   plt.plot(tt,np.nanmean(ts_test_xp5[-30:,:], axis=0),'o-',color='blueviolet',label='x0.5', alpha=0.3)
   ...
   plt.plot(tt,np.nanmean(ts_test_x2[-30:,:], axis=0)-np.nanmean(ts_test_x2[-30:,:], axis=0)[0]*0,'^-',color='b',label='x2')
   plt.plot(tt,np.nanmean(ts_test_x3[-30:,:], axis=0)-np.nanmean(ts_test_x3[-30:,:], axis=0)[0]*0,'^-',color='dodgerblue',label='x3')
   plt.plot(tt,np.nanmean(ts_test_x4[-30:,:], axis=0)-np.nanmean(ts_test_x4[-30:,:], axis=0)[0]*0,'^-',color='springgreen',label='x4')
   plt.plot(tt,np.nanmean(ts_test_x5[-30:,:], axis=0)-np.nanmean(ts_test_x5[-30:,:], axis=0)[0]*0,'^-',color='gold',label='x5')
   plt.plot(tt,np.nanmean(ts_test_x6[-30:,:], axis=0)-np.nanmean(ts_test_x6[-30:,:], axis=0)[0]*0,'^-',color='orange',label='x6')
   plt.xticks(tt,['Jul','Aug','Sep','Oct','Nov','Dec','Jan','Feb','Mar','Apr','May','Jun'], fontsize=10)
   plt.legend(ncol=1,fontsize='small',loc='upper left')
   plt.ylim(2,30)
